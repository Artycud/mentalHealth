...
**Access**
- `/admin/login` with a single account. Username in `ADMIN_USERNAME`, a password hash in `ADMIN_PASSWORD_HASH`. Never store a plain password anywhere, including `.env.example`. **Built with Node's own scrypt** (`lib/auth.ts`) rather than argon2 or bcrypt: it is just as strong for this use, and needs no native module to compile on the school's server. The stored format is `scrypt.N.r.p.salt.hash` with **no `$`**, because a `$` in `.env.local` is read as a variable and silently corrupts the value. `npm run admin:setup` makes the three values.
- With no complete config (all three, and a `SESSION_SECRET` of 32+ characters) the panel is simply closed: there is no default password and nothing to guess.
