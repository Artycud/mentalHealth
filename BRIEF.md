...
**Access**
- **One account, set up in the browser.** The first time `/admin` is opened with no account, it shows a one-time setup form (a username, a password twice) and signs the new admin in. No terminal command and no configuration. The account is stored in the `setting` table as `admin_account` (a username and a password hash); the setup route refuses (409) the moment an account exists, and one `INSERT OR IGNORE` decides a race, so two simultaneous submissions cannot both win. The password can be changed from the panel (it asks for the current one, and signs out every other session). Forgotten: `npm run admin:reset` removes the account and the setup page returns.
- **On a live server (production) setup also needs a setup code**, so the first stranger to reach `/admin` after a deployment cannot claim the account. The code (`xxxx-xxxx`, no look-alike characters) is made the first time the setup page opens, printed in the server's log, and written to `data/admin-setup-code-<database>.txt`; it is deleted when setup is done. Wrong codes lock the setup for ten minutes. While developing (`npm run dev`) no code is asked for.
- The environment can still set the account instead (`ADMIN_USERNAME`, `ADMIN_PASSWORD_HASH`, `SESSION_SECRET`, made by `npm run admin:setup`); those win over a browser-made account and are then changed there, not in the panel. Without `SESSION_SECRET` (32+ characters) the cookie-signing secret is made once and kept in the database.
- Never store a plain password anywhere, including `.env.example`. **Built with Node's own scrypt** (`lib/auth.ts`) rather than argon2 or bcrypt: it is just as strong for this use, and needs no native module to compile on the school's server. The stored format is `scrypt.N.r.p.salt.hash` with **no `$`**, because a `$` in `.env.local` is read as a variable and silently corrupts the value. Passwords must be at least 10 characters and must not contain the username.
- With no account at all the panel is simply closed: there is no default password and nothing to guess.
- On success, set a signed, `httpOnly`, `SameSite=Lax` cookie signed with `SESSION_SECRET`, expiring in 8 hours (`Secure` whenever the request is HTTPS, including behind a proxy that says so). Logout clears it. The cookie carries a fingerprint of the password hash, so changing the password signs everyone out.
...
- The booth cookie is separate from the admin cookie and grants **only**
  ...
  or any `/api/admin/*` route. It is sent only under `/booth`, and it lasts 12
  hours, so one login covers a booth day without a staff member re-entering it
  mid-service. Its login page is `/booth/login`, with its own five-tries lockout.
  Generating a new password signs out every booth device (their cookies are tied to
  the old password). An admin can open the kiosk and TV without the booth login.
- **Until the admin has generated a booth password, the kiosk and TV are open**, so a
  fresh install works at once; the panel shows a warning while they are.
- The kiosk writes sessions exactly as the phone does — anonymous, no student
  ...
  of people.
- The kiosk auto-resets to its idle screen 45 seconds after a result (any touch
  restarts the count), so a student who walks off does not leave their answer up for
  the next person.
...
| Admin set up in the browser | Needing `npm run admin:setup` to open the admin was friction, so the admin is now made in the browser on first visit (no command, no configuration). Because the panel is then claimable by whoever reaches it first, a live server also asks for a **setup code** that only someone with access to the machine can read (its log, or a file next to the database); on a developer's own computer no code is asked. Wrong codes lock the setup for ten minutes; the setup refuses once an account exists, and a simultaneous race has exactly one winner. The signing secret is made once and stored, so nothing needs configuring, and a server restart does not sign anyone out. The environment (`ADMIN_*`) still works and wins, for a server that prefers it. The password is changed in the panel, and forgotten passwords are recovered with `npm run admin:reset`. Tested by `test:auth` and `test:setup:e2e` (which was checked by disabling the code requirement on purpose and watching it fail). | 2026-09-21 |
| Admin panel and booth login | Built as the brief describes, with these decisions. **scrypt** (Node built-in) for password hashes, in a `$`-free format, instead of argon2/bcrypt: no native module for the school server to compile. **Lockout** is 5 wrong tries then 10 minutes, per account, stored in an `auth_lock` table (no address, nothing identifying) so it survives restarts and works on serverless. **CSRF**: SameSite=Lax cookies plus an Origin / Sec-Fetch-Site check on every change. **Booth screens are open until the admin makes a booth password** (the panel warns), so a fresh install works; after that they need the booth login, and regenerating the password signs out every booth device. **Security headers** (a content security policy allowing only this site, nosniff, no-referrer, no framing) are sent by the app itself, not left to the proxy. Days in the panel are Thailand days. Exports are UTF-8 with a BOM and defuse spreadsheet formulas. The wipe keeps settings, dates and accounts. A safe backup is `npm run db:backup` (SQLite `VACUUM INTO`), because copying a live database file can catch it mid-write. Tested by `test:auth`, `test:admin` and `test:admin:e2e`, which starts its own server and database and was checked by removing a guard on purpose and watching it fail. | 2026-09-20 |
...
| Redesign direction (**approved**) | The student side felt clinical: Home opened with a question about your mental state, and the check-in was a graded questionnaire ending in a severity label. The student side becomes **one seasonal place the student is in from the first frame**, with the activities as touchable objects in it: VENT (AI), MBTI (a shared axis engine with the booth flower quiz), and CUD Care. Scope for Loy Krathong: the new Home, MBTI and VENT. Full reasoning: `docs/redesign-plan.md`. | 2026-09-26 |
| Statistics (**corrected**) | §11's "the statistics the school needs for its report" overstated it. The only requirement is anonymous statistics of what students answered. The check-in is therefore not needed for reporting. | 2026-09-26 |
| User Mode look (**decided by the council lead**) | The student pages are **Mental Health Week's own look, never a festival's**: a light pink sky with soft, drifting "hearts" (blobs), from the approved home design (`components/landing`, `components/checkup`). Only the booth screens (kiosk, TV, the phone booth page) wear Loy Krathong, Christmas or CNY. The older check-in and its result take the same look through `components/heart/HeartTheme`, which only redefines the base tokens, so the booth, which shares those components, is untouched. The check-in's full→empty moon icons were dropped: they read as a gauge of how bad things are. Home asks one light thing first ("อยากรู้ไหม ใจตอนนี้เป็นยังไง" → the 2-step check-up), and content stays light: everyday questions, not deep ones. `test:look` enforces the no-festival rule on every User Mode page. A Loy Krathong-themed Home was mocked up and rejected ("it's a mental health experience, not a Loy Krathong one"). | 2026-09-26 |
| VENT, MBTI panel, lighter check-up (**built, VENT awaiting sign-off**) | **VENT** (`/vent`, `components/vent`, `lib/vent`) works as a conversation, not one turn, but not as a chat app: a heart that listens, each answer opening with the student's own words it heard (checked to be their real words), lines arriving one by one on the sky. Capped at 12 messages. Nothing is stored or logged: the conversation lives only in the page; the server keeps a per-minute count for a 60-a-minute ceiling (`vent_tick`) and nothing else; failures are logged by error name only (`test:vent` proves it, and was checked by breaking it on purpose). A deterministic safety list runs before the AI; a match, or the AI's own `urgent`, shows reviewed words with CUD Care, 1323 and 1669 instead of the AI. The AI itself is not connected yet: `VENT_PROVIDER` picks it (`mock` while developing, labelled as a test; `none` in production until a real one is added). **Before students use it: CUD Care approves the safety copy and list, the real contact channels, and the provider's data terms.** **Home**: the "บนเว็บนี้ ทำได้ตลอด" list is gone; MBTI and VENT are panels of their own (MBTI's page is still "coming soon"). **Check-up**: the breathing exercise is gone; each of the 9 hearts has its own short message (drafts); a heavy heart is offered VENT instead of a sudden CUD Care block, and CUD Care stays as a quiet link on every result. | 2026-09-26 |
| Scroll-driven home, sharper check-up, warm tone (**built**) | The home is read by scrolling: under the first screen the check-up's heart is pinned while four lines go by ("ใจเราเปลี่ยนสีได้ทุกวัน" … "ลองถามดูไหม?"), it grows and tries on colours, and reaching the end starts the first question; the pinned stretch then folds away without the page moving. A small "ถามใจเลย" link does the same for a keyboard or a hurry; /checkup keeps its button. Later sections rise in as they are reached. The captions say what the student gets ("ตอบ 2 คำถาม แล้วรู้ว่าวันนี้ใจเธอสีอะไร และมีเรื่องน่ารู้อะไรซ่อนอยู่"), never "ไม่มีถูกผิด". Questions are asked through things students know (a phone battery, a school bag). The result adds a mood map (where the heart landed) and a "รู้ไหม" fact per heart, each from a named study (drafts; keep them to what the study found). **Tone:** a slight apricot warmth over the User Mode sky, switchable: `DEFAULT_TONE` in `lib/tone.ts` is the revert, `?tone=normal|warm` previews per tab. Booth screens are never warmed. Next: per-situation or AI-written messages, using the same `message`/`fact` shape. | 2026-09-26 |
| Home as a full scroll story (**built, then set aside**: kept on the local branch `experiment/scroll-story`; main has the sectioned home with a one-page scroll intro instead, see the next row) | The home is no longer sections on a page: one stage stays pinned for 12 screens of scrolling (`components/story/HomeStory`, timings and positions in `lib/story.ts`, words in `content/th/story.ts`). Everyone's hearts live on it and act out each chapter: they gather ("ทุกคนมีใจ"), spread onto the map of hearts with its axes, step aside while the student's own heart rises for the two questions (the page settles there with a gentle scroll snap; the result puts that heart on the map among everyone's), then travel on with it: words leave it and dissolve (VENT), the hearts split into E/I, S/N, T/F, J/P (MBTI), walk a path that draws itself past the three rounds (dates only, no spoilers), and gather into one glow (CUD Care). Everything moves from the scroll position; under reduced motion nothing moves by itself. /checkup keeps the standalone, button-led check-up. | 2026-09-26 |
| One-page scroll intro, then the home (**built**) | The full scroll story was too much: the home is back to its sections (the heart panel, MBTI and VENT panels, the rounds), with one page of scroll-driven intro in front (`components/landing/IntroStory`), asked the way a friend would: everyone's hearts at the edges and "ช่วงนี้ได้ดูแลใจตัวเองบ้างไหม?"; one screen of scrolling swaps the question for the check-up's own panel ("เช็กใจตอนนี้"), with the hearts gathering around it, so the invitation and the check-up are one screen in one visual language (a second line about checking phone batteries was dropped for this). The event part ("ให้ใจได้พักบ้าง", what every round has, the rounds) now follows the MBTI and VENT panels. An earlier, more poetic intro ("ทุกคนมีใจ…") read as corny and was replaced. Kept from the scroll round: the sharper questions, the mood map and "รู้ไหม" facts in /checkup, the warm/normal tone switch, and a caption with a purpose on the heart panel. | 2026-09-26 |
| Rule reversals (**proposed, not yet in force**) | For the redesign to work, these rules would change. Until each is confirmed, the old rule stands. §8/§15 "never call an AI model": VENT calls one, server-side only, and stores nothing. §12 "no free-text box": VENT has one, and nothing typed in it is stored or logged; this **needs CUD Care's sign-off** together with the safety screen. §9 "no motion on Home": Home becomes a living scene (drifting hearts), and reduced motion still turns it all off. | 2026-09-26 |
